...
def then_v1(context):
    """Then its speed is 1, 2 or 3."""
    assert(context.star1['v'] in [1, 2, 3])
...
def then_x2(context):
    """Then its X coordinate changes according to its speed."""
    assert(context.star1['x'] == context.x + context.v)
...
def then_v2(context):
    """Then its speed does not change."""
    assert(context.star1['v'] == context.v)
...
def when_create_stars(context):
    """When I create a list with 20 stars."""
    context.stars_list = create_stars_list(20, x=800, y=[0, 600], v=[1, 2, 3])


@then('the X coordinate of each star is 800')
def then_x3(context):
    """Then the X coordinate of each star is 800."""
    for i in range(len(context.stars_list)):
        assert(context.stars_list[i]['x'] == 800)
...
def then_y3(context):
    """Then the Y coordinate of each star is between 0 and 600."""
    for i in range(len(context.stars_list)):
        assert(context.stars_list[i]['y'] >= 0 and
               context.stars_list[i]['y'] <= 600)
...
def then_v3(context):
    """Then the speed of each star is 1, 2 or 3."""
    for i in range(len(context.stars_list)):
        assert(context.stars_list[i]['v'] in [1, 2, 3])
...
def then_x4(context):
    """Then the X coordinate of each star changes according to its speed."""
    for i in range(len(context.stars_list)):
        assert(context.stars_list[i]['x'] == 800 + context.stars_list[i]['v'])
...
def then_y4(context):
    """Then the Y coordinate of each star does not change."""
    for i in range(len(context.stars_list)):
        assert(context.stars_list[i]['y'] >= 0
               and context.stars_list[i]['y'] <= 600)
...
@given(u'a specific list of stars')
def step_impl(context):
    raise NotImplementedError(u'STEP: Given a specific list of stars')


@when(u'I move the stars 4 times')
def step_impl(context):
    raise NotImplementedError(u'STEP: When I move the stars 4 times')


@then(u'the list will have only 2 stars')
def step_impl(context):
    raise NotImplementedError(u'STEP: Then the list will have only 2 stars')
